...
public class ApiDailySyncService
{
    ...
    {
        await using var connection = new SqlConnection(_replicaConnectionString);
        var lockTimeout = DateTime.UtcNow.AddHours(-1); // Consider locks older than 1 hour as stale

        var result = await connection.ExecuteAsync(@"
            UPDATE Sync_Lock 
            SET AcquiredAt = GETUTCDATE(), AcquiredBy = @MachineName
            WHERE LockName = 'DailySync' 
              AND (AcquiredAt IS NULL OR AcquiredAt < @LockTimeout)",
            new { MachineName = Environment.MachineName, LockTimeout = lockTimeout });
    ...
    private async Task ReleaseLockAsync()
    {
        ...
        await connection.ExecuteAsync(@"
            UPDATE Sync_Lock 
            SET AcquiredAt = NULL, AcquiredBy = NULL
            WHERE LockName = 'DailySync'");
    }
