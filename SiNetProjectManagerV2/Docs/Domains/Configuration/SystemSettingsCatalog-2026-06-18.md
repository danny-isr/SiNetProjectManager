...
| `InspectionTemplatesFolderId` | Inspection Templates Folder ID | Google Drive folder ID for templates | DB | Office | All | Admin | Required | - | String | Google Drive | Yes | Google Drive Reader | ID Format | Google Drive | High | "לא הוגדרה תיקיית תבניות משרדית. פנה למנהל המערכת." | "אין לך הרשאה לתיקיית התבניות ב-Google Drive. יש לבקש מהמנהל לשתף את התיקייה איתך." | "שתף את תיקיית התבניות בדרייב עם קבוצת העובדים או עם המשתמש הספציפי." | FloatingInspectionView.WireGoogleServices, TemplateValidationWindow, ManagementSettingsWindow | System Admin | No | Active | Shared setting, but folder access is per Google user. |
| `InspectionReportsFolderId` | Inspection Reports Folder ID | Google Drive folder ID for reports | DB | Office | All | Admin | Required | - | String | Google Drive | Yes | Google Drive Editor | ID Format | Google Drive | High | "לא הוגדרה תיקיית דוחות משרדית. פנה למנהל המערכת." | "אין לך הרשאה לתיקיית הדוחות ב-Google Drive. יש לבקש מהמנהל לשתף את התיקייה איתך." | "שתף את תיקיית הדוחות בדרייב עם קבוצת העובדים או עם המשתמש הספציפי." | FloatingInspectionView.WireGoogleServices, GoogleReportExportService, GoogleNoteScreenshotUploadService, ManagementSettingsWindow | System Admin | No | Active | Shared setting, per-user access required. |
| `ReportsOutputRoot` | Reports Output Root | Base UNC/Local path for reports | DB | Office | All | Admin | Optional | - | Path | UNC | Yes | Network Access | Path exists | Path | High | "נתיב דוחות לא מוגדר" | "אין גישה לנתיב" | "ודא נתיב רשת תקין" | Export | System Admin | No | Candidate for future cleanup | Needs Review. Risk if local path. |
| `InboxProjectName` | Inbox Project Name | Name of the ACC inbox project | DB | Office | All | Admin | Required | - | String | ACC | Yes | Project access | Project exists | ACC | High | "פרויקט Inbox לא מוגדר" | "אין גישה לפרויקט" | "ודא גישה לפרויקט ב-ACC" | ACC Connector | System Admin | No | Candidate for future cleanup | Needs Review. |
...
| `AccService.BaseUrl` | ACC Service Base URL | ACC API Base URL | DB | Office | All | Admin | Optional / Required only when using external AccService mode | - | URL | ACC API | No | - | Valid URL | ACC | High | "URL לא מוגדר" | - | "הגדר URL בפורמט תקין" | AppConfiguration.LoadDatabaseConfigurationOverrides, ACC service client/bootstrap code | System Admin | No | Active | Empty means local in-process ACC provisioning mode; external ACC service health check should be disabled or marked as Local mode when empty. |
...
| `AccBootstrapAdminEmail` | ACC Bootstrap Admin Email | Email of ACC bootstrap admin | DB | Office | All | Admin | Optional / Recommended when using dedicated ACC bootstrap service account | - | Email | ACC | Yes | ACC Account Admin | Email format | ACC | High | "Admin Email לא מוגדר" | - | "הגדר Email תקין של מנהל ACC" | ACC Connector | System Admin | Sensitive operational | Active | Empty currently falls back to legacy SIUser-based resolution. Existing behavior; should be reviewed in a future cleanup round. |
...
| `AiConfiguredCloudModels` | AI Configured Cloud Models | Cloud models | DB | Office | All | Admin | Optional | - | CSV (Provider\|ModelName) | None | No | - | CSV entries in Provider\|ModelName format | AI | Medium | - | - | - | AiModelCatalogWindow, AI settings / model selection logic | System Admin | No | Active | - |
| `StampTemplatePath` | Stamp Template Path | Path to stamp file | DB | Office | All | Admin | Optional | - | Path | UNC | Yes | Network Access | Path exists | Path | High | "קובץ חותמת לא הוגדר" | "אין גישה לקובץ חותמת" | "ודא נתיב רשת תקין" | QuickStampWindow.ResolveDwfTemplatePathAsync, ManagementSettingsWindow | System Admin | No | Active | Risk if local path. |
| `OfficeManagementProjectId` | Office Management Project ID | Internal project ID | DB | Office | All | Admin | Required | - | Positive integer | Database | No | - | Positive integer / existing project check | Settings | High | "פרויקט ניהול משרד לא מוגדר" | - | - | Core | System Admin | No | Active | Stored as text in SystemSettings, but semantically a positive internal project ID. |
...
| Google Drive Template Folder | Google UI / System check | Check if folder ID is readable | **OK**<br>**NotConfigured** (Admin setup issue)<br>**GoogleNotConfigured** (User connection issue)<br>**NotAuthenticated** (User connection issue)<br>**NoAccess / 403** (User external permission issue or folder sharing issue)<br>**NotFound / 404** (FolderId wrong or not visible to current Google user)<br>**EmptyFolder** (Admin/content setup issue)<br>**Error** (External service issue) | "אין לך גישה לתיקיית התבניות" | 404 or 403 response | Admin / User |
| Google Drive Reports Folder | Google UI / System check | Check if folder ID is writable | **OK**<br>**NotConfigured** (Admin setup issue)<br>**GoogleNotConfigured** (User connection issue)<br>**NotAuthenticated** (User connection issue)<br>**NoAccess / 403** (User external permission issue or folder sharing issue)<br>**NotFound / 404** (FolderId wrong or not visible to current Google user)<br>**Error** (External service issue) | "אין לך גישה לתיקיית הדוחות" | 404 or 403 response | Admin / User |
...
## 4. Sensitive vs Secret

Raw secrets **must not** be stored in `SystemSettings`. Use Vault or the existing secret mechanism for actual secrets (e.g. API Keys).
Values marked as **Sensitive operational** (like `AccBootstrapAdminEmail`) are not cryptographic secrets, but represent sensitive internal routing or bootstrap identities that should be handled with care and restricted from unnecessary logging.

## 5. How this catalog will be used

This catalog is a documentation-first artifact. It is not a new runtime authorization mechanism. It will drive future implementation of:
- **System Status / Health Check panel:** providing administrators and users with a clear diagnostic overview.
- **Admin settings validation:** ensuring configurations are valid at the time of entry.
- **User-facing diagnostic messages:** providing actionable feedback instead of raw exceptions.
- **Documentation of external permission requirements:** tracking out-of-band dependencies like Google Drive or Autodesk Construction Cloud roles.
- **Future cleanup of unused settings:** tracking deprecated or legacy keys for safe removal.
