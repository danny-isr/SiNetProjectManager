...
- Copilot / agents must **not** run `Add-Migration` / `dotnet ef migrations add`.
- Copilot / agents must **not** run `Update-Database` / `dotnet ef database update`.
- Copilot / agents must **not** create, edit, rewrite, patch, or delete migration
  files, `Designer.cs` files, or `ModelSnapshot` — **existing migration files are immutable**.
- If apply fails, diagnose only; do **not** patch `Up`/`Down`. The developer chooses the fix.
- If a schema change is required, agents must **describe** the required
  change (target model / configuration) and give the developer the EF command;
  wait for the developer to generate/apply the migration.
