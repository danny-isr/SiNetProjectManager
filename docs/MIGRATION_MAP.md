...
| ACC / Autodesk | `SiOffice.AutodeskConnector/Bim360Service.cs` (~3,231) | `SiNet.Application` `IAcc*` ports → `SiNet.Infrastructure.Autodesk` (+ optional bridge seam) | 🟡 | ACC remains source of truth; DB is cache/helper. Clean side now has the control-plane seam plus read-only `IAccProjectService` and `IAccDocumentService`, both with local/remote mode switching and matching `SiOffice.AccService` endpoints. Project discovery currently returns the known ACC IDs already recorded in SQL; it is not yet a live Autodesk account crawl. Provisioning, inbox bootstrap, filing, and metadata writes remain in the legacy host/runtime split. See [`ACC_BOUNDARY.md`](./ACC_BOUNDARY.md). |
