...
| ACC / Autodesk | `SiOffice.AutodeskConnector/Bim360Service.cs` (~3,231) | `SiNet.Application` `IAcc*` ports → `SiNet.Infrastructure.Autodesk` (+ optional bridge seam) | 🟡 | ACC remains source of truth; DB is cache/helper. Clean side now has the control-plane seam plus a real read-only `IAccDocumentService` with local/remote mode switching and a matching `SiOffice.AccService` lookup endpoint. `IAccProjectService`, provisioning, inbox bootstrap, filing, and metadata writes remain in the legacy host/runtime split. See [`ACC_BOUNDARY.md`](./ACC_BOUNDARY.md). |
