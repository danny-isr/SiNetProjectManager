...
| Identity / Permissions | `CurrentUserContext`, `UserService`, `ActionPermissionService`, `MainWindow` menu gates | `SiNet.Application.Identity` ports → Infrastructure adapters | 🟡 | **Target spec:** [`IDENTITY_AND_PERMISSIONS.md`](./IDENTITY_AND_PERMISSIONS.md). P1 auth parity + P2 profile display **implemented**. Authorization query ports **not yet**. |
