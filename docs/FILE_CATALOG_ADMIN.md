...
| `QuoteDocument` | הצעות_מחיר | תכתובת → ניהול_כספי | `.docx` | yes | Gates `PrepareQuoteDocument`; `OutSidData=false`. Office name is plural (not «הצעת_מחיר»). Set `TemplateLocation` for «אלטרנטיבה מתבנית» |
