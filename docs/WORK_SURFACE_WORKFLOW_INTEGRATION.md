...
| **ProjectWork** | Legacy `MainWindow.ShowProjectWork` | **Live** — `ProjectWorkWindowView` / `IProjectWorkSurfaceHost` (menu בעבודה 2 + task float) | `Component.ProjectWork`, `Component.MaterialChecklist`, `Component.PoliceSubmission` | Project / checklist / email source | Yes (result picker on task strip) | Workflow result codes per registry | Partial write | FileServer upload/DnD/delete; ACC write gated | **Parity gaps:** rename/delete user folders. **Catalog admin** («ניהול קבצים») is a **separate** Admin surface — see [`FILE_CATALOG_ADMIN.md`](./FILE_CATALOG_ADMIN.md). **Live:** «יצירת תיקייה»; seeded required slots `QuoteEstimate` («אומדן הצעה») + `QuoteDocument` («הצעת מחיר») under «תכתובת → ניהול כספי» (idempotent; never creates folder «הצעת מחיר»). Completion gates by **Code**: Calculation → `QuoteEstimate`; Preparation → `QuoteDocument`. Context menu «אלטרנטיבה נוספת» always prompts for name (default `1`); «אלטרנטיבה נוספת מתבנית» when `TemplateLocation` set, `OutSidData≠true`, template file exists — copies template, places, opens for edit. Visible only when project has JobType in `TypeOfProjectInProject`. |
