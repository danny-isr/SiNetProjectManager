...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **EmailWindowView** | New Shell menu (factory); legacy `MainWindow.NavigateToEmail` for filing | `ApplyContext` loads project + **direct-locates** the task email by Gmail/`rfc822msgid` (not current-page-only); see [`FILEMATERIAL_MOVETOPROJECT.md`](./FILEMATERIAL_MOVETOPROJECT.md) §12 | `Component.EmailFiling`, `Component.EmailComposeToPlanner`, `Component.ProjectCreationFromEmail`, `Component.ReviewProjectSetupFromEmail` | `EmailInboxMessage` / `EmailThread` id | Legacy: yes via coordinator after MoveToProject; New: **yes** for FileMaterial when Move + `CompleteAsync` succeed per FILEMATERIAL doc | e.g. filing completion → advance toward MaterialCheck | Read native; filing write via MoveToProject stack | Gmail read native; ACC Move native | **Pilot** — FileMaterial Target in [`FILEMATERIAL_MOVETOPROJECT.md`](./FILEMATERIAL_MOVETOPROJECT.md); dismiss only after TaskClosed + advance (not AllFilesTransferred alone) |
...
| **ProjectWork** | Legacy `MainWindow.ShowProjectWork` | **Live** — `ProjectWorkWindowView` / `IProjectWorkSurfaceHost` (menu בעבודה 2 + task float) | `Component.ProjectWork`, `Component.MaterialChecklist`, `Component.PoliceSubmission` | Project / checklist / email source | Yes (result picker on task strip) | Workflow result codes per registry | Partial write | FileServer upload/DnD/delete; ACC write gated | **Parity gaps:** rename/delete user folders. **Catalog admin** («ניהול קבצים») is a **separate** Admin surface — see [`FILE_CATALOG_ADMIN.md`](./FILE_CATALOG_ADMIN.md). **Live:** «יצירת תיקייה»; seeded required slots under «תכתובת → ניהול_כספי» (`QuoteEstimate`, `QuoteDocument`, `QuoteClientApproval`) plus optional `QuoteSendDocument` (PDF לשליחה) and nested «הצעת_מחיר» folder for `QuoteClientRequest` (PDF, `OutSidData=true`, email ACC tag). Catalog titles use underscore (not space); seed binds to existing folders/files and never overwrites `TemplateLocation` — see [`FILE_CATALOG_ADMIN.md`](./FILE_CATALOG_ADMIN.md). Completion gates by **Code**: Calculation → `QuoteEstimate`; Preparation → `QuoteDocument`; FollowQuoteApproval approve → `QuoteClientApproval`. **Orange (SOF-010):** tree highlights only catalog codes that gate the **current** task (same map as Complete); browse / MaterialCheck → no orange. Parent folders with any orange descendant stay orange even when other physical files exist in the branch. Context menu «אלטרנטיבה נוספת» always prompts for name (default `1`); «אלטרנטיבה נוספת מתבנית» when `TemplateLocation` set, `OutSidData≠true`, template file exists — copies template, places, opens for edit. Visible only when project has JobType in `TypeOfProjectInProject`. |
