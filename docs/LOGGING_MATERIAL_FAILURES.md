...
During rollout, operators diagnose from **Llog** (central Serilog share), not from each workstation’s local file. Client central default is **Warning** (`Logging.Client.CentralLevel`). After the 16.08 heartbeat ships, a healthy start **must** produce `[STARTUP] Client process alive` + sink diagnostics on Llog ([`LOGGING.md`](./LOGGING.md) §9.4.1). **1.0.32 did not meet this on the share** (local WARN only). **16.08 evening:** PROD `Logging.Client.CentralLevel` was **Error** (filtered the heartbeat); restored to Warning. Remaining code: [`DEV_DIRECTIVE_STARTUP_LOG_WRITE_VERIFY.md`](./DEV_DIRECTIVE_STARTUP_LOG_WRITE_VERIFY.md) DEV-028 Slice E (הערה if min ≠ Warning). Missing heartbeat after that ship is a **write-verify or level** miss, not “healthy silence”. Material failures still must be Warning/Error/Fatal.
