...
internal static class EmailListRowMapper
{
        ...
        Func<ProjectSummaryDto?> getCurrentProject)
    {
        ...
        if (threadLinkQuery is not null)
        {
            ...
            try
            {
                if (ids.Count > 0)
                {
                    messageLinkStates = await threadLinkQuery
                ...
                {
                    threadLinkStates = await threadLinkQuery
                        .GetLinkStatesByGmailThreadIdsAsync(threadIds)
                        .ConfigureAwait(true);
                }
            }
            catch
        ...
        Func<ProjectSummaryDto?> getCurrentProject)
    {
        ...
        var isLinked = messageLink?.IsLinked == true || threadLink?.IsLinked == true || isLinkedFromLabels;
        var projectDisplay = isLinked
            ? messageLink?.DisplayName ?? threadLink?.DisplayName ?? labelProjectName ?? summary.PrimaryLabel ?? "משויך"
            : "לא משויך";
        ...
        return new EmailListRow(
            ...
            ProjectNumber: messageLink?.ProjectNumber ?? threadLink?.ProjectNumber,
            ProjectName: messageLink?.ProjectName ?? threadLink?.ProjectName ?? labelProjectName,
