...
public sealed class NewShellNativeUserAdminMenuTests
{
    ...
    {
        var items = BuildMenuItems(usersManage: true);
    ...
    {
        var items = BuildMenuItems(usersManage: false);
    ...
    private static IReadOnlyList<NewShellMenuItem> BuildMenuItems(bool usersManage)
    {
        var services = new ServiceCollection();
        services.AddSingleton<IAuthorizationQueryService>(new StubAuthorization(usersManage));
        services.AddTransient<UserListWindow>(_ => new UserListWindow(new UserManagementViewModel(new NoOpUserManagementService())));
        services.AddTransient<AddUserDialogWindow>(_ => new AddUserDialogWindow(new AddUserViewModel(new NoOpUserManagementService())));
