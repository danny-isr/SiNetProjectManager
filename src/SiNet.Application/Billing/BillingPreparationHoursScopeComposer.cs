...
public static class BillingPreparationHoursScopeComposer
{
    ...
    /// <summary>
    /// Groups hour lines that share the same inclusive date window so task text can
    /// list multiple sub-contracts under one period block.
    /// </summary>
        ...
        IReadOnlyList<BillingPreparationHoursLineSnapshot> hours)
    {
        ...
        if (hours.Count == 0)
            return Array.Empty<IReadOnlyList<BillingPreparationHoursLineSnapshot>>();

        return hours
            .GroupBy(h => (From: h.FromDate.Date, To: h.ToDate.Date))
            .OrderBy(g => g.Key.From)
            .ThenBy(g => g.Key.To)
            .Select(g => (IReadOnlyList<BillingPreparationHoursLineSnapshot>)g
                .OrderBy(h => h.SubContractName, StringComparer.OrdinalIgnoreCase)
                .ThenBy(h => h.MasterPlanSubContractId)
                .ToList())
            .ToList();
    }
}
