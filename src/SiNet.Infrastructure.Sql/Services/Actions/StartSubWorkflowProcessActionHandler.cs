        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var (advancedInstance, tasks) = await _provisioning.EnsureInitialStageTasksAsync(
                subInstance, command.UserId ?? 0, cancellationToken).ConfigureAwait(false);
        ...
        catch (Exception ex)
        {
            ...
            return ActionExecutionResultDto.Failed(ActionCode, $"Failed to start sub-workflow: {ex.Message}");
        }
    }
}
