...

{
    ...
    [
        ...
        new("DEBUG_TASK_SEED_NO_PRIORITY", $"{TitlePrefix} Medium no-priority (closed)", WorkQueueBucketCodes.Medium, null, false, "דמו — ללא priority (סגור)", 931),
    ...
    public async Task<SeedResult> SeedAsync(DemoTaskSeedOptions? options = null, CancellationToken ct = default)

    {

        options ??= new DemoTaskSeedOptions();
...

        {
            ...
            var (user, userWarning) = await ResolveTargetUserAsync(db, options, ct).ConfigureAwait(false);

            if (user is null)

                return Fail(userWarning ?? "No target user for demo seed.");



            var project = await ResolveTargetProjectAsync(db, options, ct).ConfigureAwait(false);

            if (project is null)

                return Fail($"Target project {options.TargetProjectId} not found.");
...

            {
                ...
                var task = CreateTask(spec, project.Id, user.Id, statusId, taskType.Id, now);

                if (spec.RequiresOpenStatus && openStatus.IsActionable)
...

            {
                ...
                await TaskQueuePriorityEngine.InsertWithAutoPriorityAsync(db, resolveTask, ct).ConfigureAwait(false);
...

            {
                ...
                await TaskQueuePriorityEngine.ValidateAndReindexDetailedAsync(db, user.Id, bucket, ct).ConfigureAwait(false);

            }



            LogDemoTaskDiagnostics(db, user.Id, project.Id);



            var summary = createdCount == 0

                ? $"Demo tasks already present for user {user.Id} (idempotent — no new rows)."

                : $"Created {createdCount} demo task(s) for user {user.Id} on project {project.Id}.";



            if (!string.IsNullOrEmpty(userWarning))

                summary = $"{userWarning} {summary}";
    ...
    private static async Task<(Siuser? User, string? Warning)> ResolveTargetUserAsync(

        SiNetSQLDbContext db,

        DemoTaskSeedOptions options,

        CancellationToken ct)

    {

        if (options.TargetUserId is int targetId)

        {

            var user = await db.Siusers

                .FirstOrDefaultAsync(u => u.Id == targetId && u.IsActive, ct)
            ...
            if (user is null)
                ...
                return (null, $"Target user {targetId} not found or inactive.");
        ...
        if (options.RequireCurrentUser)
            ...
            return (null, "No target user specified. Log in as a user before loading demo tasks.");



        var fallback = await db.Siusers
        ...
        if (fallback is null)
            ...
            return (null, "No active user found in database — demo tasks require an existing Siuser.");



        return (fallback, $"No current user; used first active user {fallback.Id}.");

    }



    private static async Task<Project?> ResolveTargetProjectAsync(
...

    {
        ...
        if (options.UseCurrentProject && options.TargetProjectId is int pid and > 0)

        {

            return await db.Projects.FirstOrDefaultAsync(p => p.Id == pid, ct).ConfigureAwait(false);

        }



        return await FindOrCreateDemoProjectAsync(db, ct).ConfigureAwait(false);

    }



    private static async Task<Project> FindOrCreateDemoProjectAsync(SiNetSQLDbContext db, CancellationToken ct)
    ...
    private static void LogDemoTaskDiagnostics(SiNetSQLDbContext db, int userId, int projectId)

    {

        var demoTasks = db.ProjectAssignments
            ...
            .Select(t => new { t.Id, t.Title, t.WorkQueueBucket, t.AssignedToId, t.ProjectId })
        ...
        DevToolsLog.Info(
            ...
            $"[DemoSeed] user={userId} project={projectId} demoTasks={demoTasks.Count} " +

            $"buckets=Quick:{demoTasks.Count(t => t.WorkQueueBucket == WorkQueueBucketCodes.Quick)} " +

            $"Medium:{demoTasks.Count(t => t.WorkQueueBucket == WorkQueueBucketCodes.Medium)} " +

            $"Long:{demoTasks.Count(t => t.WorkQueueBucket == WorkQueueBucketCodes.Long)}");
    ...
    private static ProjectAssignment CreateTask(
        ...
        CreateTask(spec.Title, spec.Bucket, null, projectId, assignedToId, statusId, taskTypeId, now);
