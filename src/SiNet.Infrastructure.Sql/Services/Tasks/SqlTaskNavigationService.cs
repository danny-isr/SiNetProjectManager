...
public sealed class SqlTaskNavigationService : ITaskNavigationService
{
    ...
    public async ValueTask<WorkSurfaceContext?> ResolveAsync(int taskId, CancellationToken ct)
    {
        ...
        var (_, primaryId, _, _) = ResolveWorkTargets(task, interaction);
    ...
    private static (TaskWorkTargetEntityType Type, long? PrimaryId, IReadOnlyList<long> All, IReadOnlyList<long> Pending)
        ResolveWorkTargets(ProjectAssignment task, TaskInteractionDefinition interaction)
    {
        ...
        if (workTargets.Count > 0)
        {
            ...
            var pendingIds = workTargets
                ...
                .ToArray();
            long? primary = pendingIds.Length > 0 ? pendingIds[0] : allIds[0];
            return (interaction.PrimaryWorkTargetEntityType, primary, allIds, pendingIds);
        }

        if (candidates.Count == 0)
            return (interaction.PrimaryWorkTargetEntityType, null, Array.Empty<long>(), Array.Empty<long>());

        var ids = candidates.Select(l => l.LinkedEntityId).ToArray();
        return (interaction.PrimaryWorkTargetEntityType, ids[0], ids, ids);
    }
